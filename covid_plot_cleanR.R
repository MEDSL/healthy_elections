...
cases_plot_all <- ggplot(ts_covid_all, aes(x = Date, y = total_cases)) + 
  ...
  geom_vline(aes(xintercept=as.numeric(sort(unique(ts_covid$Date))[64]), color="#37C256" ), linetype=4, show.legend = F) + 
  geom_vline(aes(xintercept=as.numeric(sort(unique(ts_covid$Date))[77]), color="#C0BA79" ), linetype=5, show.legend = F) + 
  geom_vline(aes(xintercept=as.numeric(sort(unique(ts_covid$Date))[38]), color="#F6573E" ), linetype=2, show.legend = F)
