...
wd <- "F:/MEDSL/healthy_elections/AZ" # set working directory 
setwd(wd)
covid_cases <- read.csv("F:/MEDSL/healthy_elections/us-counties-nyt.csv") # read in the COVID data from the NYT 
covid_cases <- subset(covid_cases, state=="Arizona") ###subset the data to equal the state of interest 
...
# April 7 is 4 weeks post primary 
# MArch 19 is executive order on COVID 
# March 17 is primary 
...
ts_covid_all <- ts_covid_all[1:73,] #entry 65 is the 2 week after primary, so 14 days later is 4 weeks after primary
sort(unique(ts_covid_all$date))

##get dates 
as.numeric(sort(unique(ts_covid_all$Date))[54])
as.numeric(sort(unique(ts_covid_all$Date))[52])
as.numeric(sort(unique(ts_covid_all$Date))[66])

cases_plot_all <- ggplot(ts_covid_all, aes(x = Date, y = new_cases)) + 
  geom_line(color="#156DD0", size=1)+  
  geom_vline(aes(xintercept=18352, color="#4E4A81" ), linetype=3, show.legend = F, lwd=1.4) + 
  geom_vline(aes(xintercept=18338, color="#37C256" ), linetype=2, show.legend = F, lwd=1.4) 
cases_plot_all
#cases_plot_all
#grob_prim_o <- grobTree(textGrob("Original \nPrimary Date", x=0.01,  y=0.7, hjust=0,
 #                              gp=gpar(col="black", fontsize=12, fontface="bold")))
grob_prim <- grobTree(textGrob("Primary", x=0.55,  y=0.7, hjust=0,
                               gp=gpar(col="black", fontsize=12, fontface="bold")))
grob2weeks <-   grobTree(textGrob("2 Weeks \npost-primary", x=0.7,  y=0.95, hjust=0,
                                  gp=gpar(col="black", fontsize=12, fontface="bold")))
cases_plot_all <- cases_plot_all +    annotation_custom(grob_prim) +  annotation_custom(grob2weeks) +
...
cases_plot_all
ggsave("covid_az_plot.jpg", plot = cases_plot_all, scale = 1,
