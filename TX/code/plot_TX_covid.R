...
library(showtext)
font_import("/Users/cantstopkevin/Documents/HarvardDesktop/MEDSL/github/healthy_elections/styrene_b/")  #specify font path here 
windowsFonts(A = windowsFont("styrene_b"))
options(stringsAsFactors = F)
wd <- "/Users/cantstopkevin/Documents/HarvardDesktop/MEDSL/github/healthy_elections/TX/covid" # set working directory 
setwd(wd)
...
covid_primary_date <- subset(covid_cases, date=="2020-03-15") # set the date to the day of the primary to run a scatterplot of some sort , then merge onto
...
cases_plot_all <- ggplot(ts_covid_all, aes(x = Date, y = total_cases)) + 
  geom_line(color="#156DD0", size=1)+  
  geom_vline(aes(xintercept=as.numeric(sort(unique(Date))[64]), color="#37C256" ), linetype=4, show.legend = F) + 
  geom_vline(aes(xintercept=as.numeric(sort(unique(Date))[77]), color="#C0BA79" ), linetype=5, show.legend = F) + 
  geom_vline(aes(xintercept=as.numeric(sort(unique(Date))[38]), color="#F6573E" ), linetype=2, show.legend = F)
...
cases_plot_all + theme(title = element_text(size = rel(1.4), family="Styrene B"))
...
ts_covidA <- covid_cases %>% group_by(date,cases) %>% dplyr::summarize(total_cases = sum(cases,na.rm=T),total_deaths=sum(deaths,na.rm=T))
